...
async def kick(message, member: discord.User = None, *, reason='No reason given'):
    if member != None:
        channel = bot.get_channel(836955773513760769)
...
async def ban(message, member: discord.User, *, reason='No reason given'):
    if member != None:
        channel = bot.get_channel(836955773513760769)
        ...
        dm = discord.Embed(title=":man_police_officer::lock: You were banned from {}".format(message.author.guild.name),
                           colour=discord.Color.from_rgb(220,20,60), timestamp=datetime.datetime.utcnow())
        dm.add_field(name="Reason", value=reason)
        p = await member.create_dm()
    ...
    if member != "":
        ...
        for ban_entry in banned_users:
            ...
            if (user.name, user.discriminator) == (member_name, member_discriminator):
                ...
                em = discord.Embed(
                    description=":man_police_officer::unlock: **Unbanned {}#{}!**".format(user.name, user.discriminator),
...
@bot.command()
@commands.has_permissions(administrator=True)
async def mute(message, member : discord.Member, reason='No reason given'):
    if member != None:
        channel = bot.get_channel(836955773513760769)
...
@bot.command()
@commands.has_permissions(administrator=True)
async def tempmute(message, member : discord.Member, time, *, reason='No reason given'):
    if member != None:
        channel = bot.get_channel(836955773513760769)
        ...
        if timesec != 'Error':
            ...
            await message.channel.send("**:x: I can not understand this amount of time, check `#help tempmute` for syntax.**")
    else:
        await message.channel.send("**:x: You can not tempmute yourself! :rofl:**")

@bot.command()
@commands.has_permissions(ban_members=True)
async def tempban(message, member: discord.Member, time, *, reason='No reason given'):
    if member != None:
        channel = bot.get_channel(836955773513760769)
...
async def clear(message, num = '100'):
    if num == "all":
...
@bot.command()
@commands.has_permissions(administrator=True)
async def clearmoney(message, member : discord.Member = None):
    if member == None:
        member_data = load_member_data(message.author.id)
        member_data.wallet = 0
        member_data.bank = 0
        save_member_data(message.author.id, member_data)
        await message.channel.send(f":white_check_mark: **Cleared {message.author.mention}'s account successfully!**")
    else:
