...
async def dice(self, message, amt=0):
    ...
    if wallet_money >= amt > 0:
        ...
        if amt <= 0:
            await message.send("Enter a **valid** number")
        else:
            await message.send(":x: **You don't have enough money in your wallet**")

async def slots(self, message, amt=None):
    member_data = load_member_data(message.author, message.guild)
    if amt is None or int(amt) <= 0:
        await message.send(":x: **Enter a valid number!**")
        return
        ...
        if amt > 0 and loaded_rounds > 0:
            ...
            if amt <= member_wallet and loaded_rounds < 6:
                if not member_data["isPlayingRoulette"]:
                    ...
                    if not random.choice(chamber):
                        ...
                        while True:
                            ...
                            while msg.content not in ['continue', 'stop'] or not check(msg):
                                await ctx.send(":x: **I expected something from `continue` or `stop`**")
                                msg = await self.bot.wait_for('message', check=check)
                    ...
                    await ctx.send("You are already playing roulette!")
            else:
                await ctx.send("You don't have such an amount of money in your wallet!")
        else:
            await ctx.send("Enter **valid** numbers!")
    else:
        await ctx.send("Enter **valid** values!")
...
async def rob(self, message, member: Member = None):
    if member is not None and member != message.author:
        ...
        if mwbal > 0:
            ...
            await message.send(f":x: **{member.mention} does not have anything in his wallet.**")
    else:
        if member is None:
            await message.send(":x: **Please write someone to rob))**")
        else:
            await message.send(":x: **You can not rob yourself! :rofl:**")
